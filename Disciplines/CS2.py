...
class CS:
    def __init__(self, appname):
        ...
        self.liquipedia = LPRequest(appname, "counterstrike")
    ...
    async def get_tournament(self):
            ...
            for row in rows:
                ...
                if tournament['tournament'] not in tournaments_db_names:
                        cursor.execute(
                            ...
                            ('CS2', tournament['tournament'], tournament['tier'], tournament['prize'],
                             tournament['teams_count']))
        for tournament_db in tournaments_db:
            if tournament_db[0] not in tournaments_names and tournament_db[1] == 'CS2':
        ...
        cursor_main.execute('DELETE FROM TournamentsInfo WHERE Game = ?', ('CS2',))

...
class Apex:
    def __init__(self, appname):
        ...
        self.liquipedia = LPRequest(appname, "apexlegends")
    ...
    async def get_tournament(self):
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                tournament_link = row.find("div", class_="gridCell Tournament Header")
                tournament["tier"] = tournament_tier.get_text().split()[0]
                for i in tournament_link.select("a[href*=apexlegends]"):
                    if len(("liquipedia.net" + i["href"]).split("/")) != 3:
                        tournament["link"] = "liquipedia.net" + i["href"]
                ...
                if tournament['tournament'] not in tournaments_db_names:
                    cursor.execute('INSERT INTO TournamentsInfo (Game, Tournament, Tier, Prize, TeamsCount) VALUES (?, ?, ?, ?, ?)',
                                ('Apex Legends', tournament['tournament'], tournament['tier'], tournament['prize'], tournament['teams_count']))
                tournaments.append(tournament)
        for tournament_db in tournaments_db:
            if tournament_db[0] not in tournaments_names and tournament_db[1] == 'Apex Legends':
        ...
        cursor_main.execute('DELETE FROM TournamentsInfo WHERE Game = ?', ('Apex Legends',))
        ...
        for match in table:
            ...
            if (datetime.today() - parser.parse(date_string)) <= timedelta(days=2, hours=12):
                ...
                if tz_string != "PET":
                    ...
                    date_res = date_upd + timedelta(hours=tz_diff(date_upd, tz, tz_needed))
                else:
